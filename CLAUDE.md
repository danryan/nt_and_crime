...
2. Deploy Verifier (and the under-test if not already installed). Free the USB-MIDI port first (the human disconnects nt_helper; `deploy-sysex` hard-fails while it is held), then `make deploy-sysex SYSEX_PLUGIN=build/arm/Verifier.o SYSEX_ID=0`. Always `SYSEX_ID=0`.
