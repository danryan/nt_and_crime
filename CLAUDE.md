...
When porting a new O_C app, pick the reference by its button handling: copy `Harrington1200.cpp` if the vendor app reads `EVENT_BUTTON_LONG_PRESS`, else copy `Low_rents.cpp`. The Harrington1200 customUi maps the runtime `LONG_RELEASE` classification to the vendor `EVENT_BUTTON_LONG_PRESS` the handler tests for; Low_rents has no long-press path. FPART reads long-press on DOWN (page toggle) and L (copy/paste), so it follows the Harrington1200 mapping.
