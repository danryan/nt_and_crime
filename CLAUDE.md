...
nt_helper MCP has no plug-in-file-upload tool (preset/param/slot/routing/screen only). Prefer uploading `.o` files through the nt_helper desktop app's plug-in upload; it already owns the USB-MIDI port, so there is no disconnect/reconnect juggling. Use `make deploy-sysex` only as the fallback (when the app is unavailable or scripting the deploy), and note it needs exclusive USB-MIDI so the app/MCP port must be freed first. Its `edit_parameter` rejects numeric values ("requires a numeric value") because the untyped value field is stringified in transit; only enum-string edits work over MCP. Batch `new([a,b])` intermittently drops the second algorithm ("did not appear"); a single `add()` or a retry is reliable. nt_helper builds its algorithm catalog at connect and does NOT refresh it after a device reboot or plug-in rescan: `new`/`add` by a freshly-deployed GUID returns "Algorithm with GUID X not found in the list of available algorithms" even though the device has registered it. This is an nt_helper-side staleness artifact, not device truth; run nt_helper's rescan-plugins command to re-enumerate (a plain disconnect/reconnect did NOT refresh the catalog in practice), or confirm registration on the hardware UI.
...
`make deploy-sysex` (`push_plugin_to_device.py`) needs exclusive USB-MIDI. While the nt_helper MCP client holds the port the upload hard-fails ("Error uploading plug-in file!") on every attempt, not intermittently; free the port (disconnect nt_helper), upload, then reconnect. The script runs `newPreset()` before uploading, so a failed upload blanks the device preset. A sysex `0x7F` reboot (send via `mido`) reloads plug-in code and re-caches the SRAM requirement; the device auto-loads its last preset on boot. The upload is also occasionally flaky with the port free: in a multi-`.o` batch, an individual upload can fail with the same "Error uploading plug-in file!" while its neighbors succeed. Retry the failed `.o` after a short pause; that transient case is distinct from the held-port case, which fails on every attempt.
...
2. Deploy Verifier (and the under-test if not already installed). Prefer the nt_helper desktop app's plug-in upload: it owns the port, so no disconnect/reconnect, and the human drives it in the app. Fallback (app unavailable or scripted): free the USB-MIDI port first (the human disconnects nt_helper; `deploy-sysex` hard-fails while it is held), then `make deploy-sysex SYSEX_PLUGIN=build/arm/Verifier.o SYSEX_ID=0`. Always `SYSEX_ID=0`.
