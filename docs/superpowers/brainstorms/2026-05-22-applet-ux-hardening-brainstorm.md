...
**2026-05-23 update: fixed via `step()`-side overdraw on `dr/q2-spike-overdraw` (follow-up batch).** Phase-1 spike inverted bottom rows of `NT_screen` from `step()`; the inversion was visible both during and outside the firmware overlay, proving `step()` writes survive the firmware's overlay pass. Phase 3 implemented cache+restore: `draw()` snapshots the bottom 16 rows into per-instance SRAM at the end of the host render; `step()` restores those bytes after every audio buffer for the next `kFooterRestoreSteps=100` calls (the guard prevents corrupting another algorithm's display after the user navigates away). Cache size tuning: 8 rows left a sliver of overlay visible; 16 rows fully covers it. SRAM-size caching note: after enlarging `_HHInstance` / `_QQInstance`, the device requires a power cycle so the firmware re-runs `calculateRequirements`; without it the cache buffer overruns the prior allocation and the restore reads corrupted bytes. Applied to both hosts (`Hemispheres_host.cpp` + `Quadrants_host.cpp`). The upstream feature request still stands: a firmware-level `kNT_tagSuppressFooterHints` would be cleaner than our overdraw workaround.
