...
#include "../../shim/include/applet_manifests/ADSREG.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/ADSREG.h"
#include "_per_applet_runtime.h"

// ManifestNS aliases the manifest struct; the vendor class is also named ADSREG.
// Using a local alias avoids ambiguity between per_applet::ADSREG and ::ADSREG.
namespace { using ManifestNS = per_applet::ADSREG; }
...
// Test seam: clears the enc_edit global so tests that drive encoder navigation
// start from a known-off edit mode regardless of prior test side effects.
void adsreg_reset_edit_mode() {
    HS::enc_edit[LEFT_HEMISPHERE].isEditing = false;
}
