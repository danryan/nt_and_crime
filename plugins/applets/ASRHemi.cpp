// NT_HEM_NEED_HEAP_ARENA must be defined before ANY #include so that
// cxx_runtime_stubs.cpp (aggregated by hem_shim_impl.h) emits the bump-
// allocator operator new instead of the nullptr stub. ASR.h includes
// HSRingBufferManager.h, whose RingBufferManager::get() does
// `new RingBufferManager` (a one-time singleton); without the arena
// that call returns nullptr and the device hard-faults on add.
...
#include "../../shim/include/applet_manifests/ASRHemi.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/ASR.h"
#include "_per_applet_runtime.h"

// Build token is ASRHemi to avoid collision with plugins/apps/ASR.o and
// build/arm/ASR.o (the OC full-screen app). The manifest name is still "ASR".
using Manifest = per_applet::ASRHemi;

// kNumParams = 4 inputs + 2 outputs * 2 (bus + mode) = 8.
static constexpr int kNumParams = 4 + 2 * 2;

static _NT_parameter s_params[kNumParams];  // [0]=Clock gate, [1]=Freeze gate, [2]=CV cv, [3]=Index cv, [4..5]=OutA, [6..7]=OutB
...
static void on_aux_button_impl(_NT_algorithm* /*self*/) {
    // ASR has no aux button action in standalone view.
}
...
static void parameterChanged_impl(_NT_algorithm* /*self*/, int /*p*/) {
    // No applet-specific parameter remapping for ASR.
}
