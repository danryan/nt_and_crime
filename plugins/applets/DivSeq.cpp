...
// all in scope when DivSeq.h (and its dep util/clkdivmult.h) is parsed.
#include "../../shim/include/HemisphereApplet.h"
#include "../../shim/include/HemiPluginInterface.h"
#include "../../shim/include/applet_manifests/DivSeq.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/DivSeq.h"
#include "_per_applet_runtime.h"

// Use a distinct alias to avoid shadowing the DivSeq applet class.
using Manifest = per_applet::DivSeq;
...
static void on_aux_button_impl(_NT_algorithm* self) {
    (void)self;
    // DivSeq's AuxButton toggles the step mute at the current cursor position.
    // In standalone mode the aux button is not claimed (kNT_button1 is not in
    // hasCustomUi), so this function is not reachable from firmware in practice.
}
...
static void parameterChanged_impl(_NT_algorithm* /*self*/, int /*p*/) {
    // No applet-specific parameter remapping for DivSeq.
}
