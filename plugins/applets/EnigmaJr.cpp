// EnigmaJr per-applet NT plug-in.
//
...
// HSMIDI.h shim provides usbMIDI no-op stubs needed by EnigmaOutput.h.
// user_turing_machines[40] is defined inside TuringMachine.h as a DMAMEM
// (no-op on this build) global — shared singleton by vendor design.
//
// Bus layout (from vendor SetHelp):
//   inputs[0] = Clock (gate)
//   inputs[1] = Reset (gate)
//   inputs[2] = Shift / transpose CV (cv)
//   inputs[3] = Organize CV (cv)
//   outputs[0] = Out A (cv)
//   outputs[1] = Out B (cv)
//
// 10x ticks-per-step note: Clock(0) and Clock(1) advance state machine
// inside the 10 inner ticks. The Turing Machine register advances once per
// Clock(0) rising edge but the edge assertion repeats 10x. Tests use
// round-trip and state-injection rather than bus-level fire-count assertions.

#include "../../shim/include/HemisphereApplet.h"
#include "../../shim/include/PhzIcons.h"
#include "../../shim/include/Arduino.h"
#include "../../shim/include/HSMIDI.h"
#include "../../shim/include/HemiPluginInterface.h"
#include "../../shim/include/applet_manifests/EnigmaJr.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/EnigmaJr.h"
#include "_per_applet_runtime.h"

namespace { using ManifestNS = per_applet::EnigmaJr; }
