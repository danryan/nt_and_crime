...
#include "../../shim/include/applet_manifests/Metronome.h"
#include "_per_applet_runtime.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/Metronome.h"

namespace { using ManifestNS = per_applet::Metronome; }
...
// Test seam: Metronome::OnDataRequest() always returns 0 (no serialisable state).
uint64_t metronome_applet_on_data_request(_NT_algorithm* self) {
    return static_cast<_AppletInstance*>(self)->applet.OnDataRequest();
}

// Test seam: expose cursor field via OnButtonPress side effect (cursor toggles 0/1).
// Returns the applet pointer so tests can drive encoder interactions.
