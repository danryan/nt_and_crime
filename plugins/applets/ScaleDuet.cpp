...
#include "../../shim/include/applet_manifests/ScaleDuet.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/ScaleDuet.h"
#include "_per_applet_runtime.h"

// ScaleDuet manifest name == vendor class name; alias the manifest.
namespace { using ManifestNS = per_applet::ScaleDuet; }
...
// Test seam: reads mask[scale] from the vendor applet.
uint16_t scaleduet_applet_get_mask(_NT_algorithm* self, int scale) {
    auto* inst = static_cast<_AppletInstance*>(self);
    uint64_t packed = inst->applet.OnDataRequest();
    if (scale == 0) return static_cast<uint16_t>(packed & 0xFFF);
    return static_cast<uint16_t>((packed >> 12) & 0xFFF);
}
