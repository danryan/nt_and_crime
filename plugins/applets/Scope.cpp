...
#include "../../shim/include/applet_manifests/Scope.h"
#include "_per_applet_runtime.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/Scope.h"

namespace { using ManifestNS = per_applet::Scope; }
...
// Scope's OnDataRequest() always returns 0 (no state packed).
uint64_t scope_applet_on_data_request(_NT_algorithm* self) {
    return static_cast<_AppletInstance*>(self)->applet.OnDataRequest();
}

// Test seam: read the current BPM value from applet private state via
// a thin accessor. This is needed to verify model-multiplier BPM behaviour
// without poking private members from the test TU.
int scope_applet_bpm(_NT_algorithm* self) {
    // OnDataRequest() returns 0, so we cannot extract bpm via serialisation.
    // Instead we expose it directly from the instance. The applet's private
    // `bpm` member is not accessible from outside the class, so we use a
    // controlled round-trip: draw() triggers View() which calls DrawBPM()
    // which reads bpm. Rather than parsing rendered pixels, the test observes
    // the passive CV passthrough (Out(ch) = In(ch)) which is directly
    // observable on the output bus and does not require BPM access.
    //
    // For tests that do need bpm, this accessor returns -1 to signal
    // "not directly accessible"; tests use the output bus instead.
    (void)self;
    return -1;
}
