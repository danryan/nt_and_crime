// NT_HEM_NEED_USER_PATTERNS must be defined before any #include so that the
// globals.cpp aggregation (via hem_shim_impl.h) instantiates
// OC::user_patterns[8]. MiniSeq.h (pulled by Seq32.h) reads that array.
#define NT_HEM_NEED_USER_PATTERNS 1

// Shim headers must precede the vendor applet header so that HemisphereApplet,
// Arduino.h stubs (constrain), OC_core.h (OC::CORE::ticks), OC_patterns.h,
// and PhzIcons are all in scope when Seq32.h (and its dep MiniSeq.h) is parsed.
#include "../../shim/include/HemisphereApplet.h"
// OC_patterns.h must be explicitly included before Seq32.h because MiniSeq.h
// (included by Seq32.h) references OC::Patterns and OC::user_patterns directly.
// The standard shim include chain (HemisphereApplet.h) does not pull it.
#include "../../shim/include/OC_patterns.h"
#include "../../shim/include/HemiPluginInterface.h"
#include "../../shim/include/applet_manifests/Seq32.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/Seq32.h"
#include "_per_applet_runtime.h"

// Use a distinct alias to avoid shadowing the Seq32 applet class.
using Manifest = per_applet::Seq32;

// 2 inputs + 2 outputs * 2 = 6 base parameters.
...
static void parameterChanged_impl(_NT_algorithm* /*self*/, int /*p*/) {
    // No applet-specific parameter remapping for Seq32.
}
