// NT_HEM_NEED_USER_PATTERNS MUST be the first line: it gates instantiation of
// OC::user_patterns[8] inside globals.cpp (aggregated via hem_shim_impl.h).
// Without it, every MiniSeq dereferences a zero-initialised extern and the
// note-buffer pointer is nullptr.
#define NT_HEM_NEED_USER_PATTERNS 1

#include "../../shim/include/HemisphereApplet.h"
// OC_patterns.h must be included explicitly before the vendor applet header so
// that OC::Patterns and OC::user_patterns are visible to MiniSeq.h (which is
// included transitively by SeqPlay7.h and uses those names at point of parse).
#include "../../shim/include/OC_patterns.h"
#include "../../shim/include/HemiPluginInterface.h"
#include "../../shim/include/applet_manifests/SeqPlay7.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/SeqPlay7.h"
#include "_per_applet_runtime.h"

using Manifest = per_applet::SeqPlay7;
...
static void parameterChanged_impl(_NT_algorithm* /*self*/, int /*p*/) {
    // No applet-specific parameter remapping for SeqPlay7.
}
