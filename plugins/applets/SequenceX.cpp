...
// all in scope when SequenceX.h is parsed.
#include "../../shim/include/HemisphereApplet.h"
#include "../../shim/include/HemiPluginInterface.h"
#include "../../shim/include/applet_manifests/SequenceX.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/SequenceX.h"
#include "_per_applet_runtime.h"

// Use a distinct alias to avoid shadowing the SequenceX applet class.
using Manifest = per_applet::SequenceX;
...
static void on_aux_button_impl(_NT_algorithm* self) {
    (void)self;
    // SequenceX has an AuxButton (mute toggle) but kNT_button1 is not claimed
    // in standalone per CLAUDE.md "Per-applet standalone customUi mapping".
}
...
static void parameterChanged_impl(_NT_algorithm* /*self*/, int /*p*/) {
    // No applet-specific parameter remapping for SequenceX.
}
