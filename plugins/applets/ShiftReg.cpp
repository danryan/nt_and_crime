...
// all in scope when ShiftReg.h (and its dep braids quantizer) is parsed.
#include "../../shim/include/HemisphereApplet.h"
#include "../../shim/include/HemiPluginInterface.h"
#include "../../shim/include/applet_manifests/ShiftReg.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/ShiftReg.h"
#include "_per_applet_runtime.h"

// Use a distinct alias to avoid shadowing the ShiftReg applet class.
using Manifest = per_applet::ShiftReg;
...
static void on_aux_button_impl(_NT_algorithm* self) {
    (void)self;
    // ShiftReg has no aux button action.
}
...
static void parameterChanged_impl(_NT_algorithm* /*self*/, int /*p*/) {
    // No applet-specific parameter remapping for ShiftReg.
}
