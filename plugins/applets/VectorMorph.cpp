// _per_applet_runtime.h provides all shim implementations (globals, graphics,
// HemisphereApplet base class, etc.) but does NOT include HemispheresFactory.h
// or the vendor applet headers. VectorMorph needs the vector-osc dep headers
// pre-included from the shim so the vendor applet's relative includes
// `../vector_osc/HSVectorOscillator.h` etc. hit their include guards and
// skip the vendor copies, avoiding duplicate-symbol link failures.
#include "_per_applet_runtime.h"

// Manifest included before the vendor applet header so that
// per_applet::VectorMorph (the manifest struct) is visible before the VectorMorph
// class declaration. We capture the manifest type into an alias immediately.
#include "../../shim/include/HemiPluginInterface.h"
#include "../../shim/include/applet_manifests/VectorMorph.h"

// Capture alias before the vendor class shadows the unqualified name.
using AppletManifest = per_applet::VectorMorph;

// Vector-osc prereqs: DMAMEM, InterpLinear16, user_waveforms declaration.
// Must precede the vendor VectorMorph.h (which does relative includes of these).
...
#include "../../vendor/O_C-Phazerville/software/src/applets/VectorMorph.h"
...
// ---------------------------------------------------------------------------
// Opaque extern "C" accessors for test seam (test TU cannot include vendor
// header due to ODR collision with this TU).
// ---------------------------------------------------------------------------
