// hMIDIOut per-applet plug-in: CV -> MIDI.
//
// The vendor Controller() reads Gate(0) plus In(0) (pitch) and In(1) (a
// selectable second function) and emits MIDI through HS::frame.MIDIState.Send*
// (Layer 0b -> NT_sendMidi*). It produces no CV output. The firmware delivers
// no MIDI into this applet, so the factory midiMessage / midiRealtime fields
// stay null.
//
// 10x clocked-multiplier note (CLAUDE.md "Critical gotcha"): the runtime runs
// Controller() ~10 times per buffer. hMIDIOut uses the ADC-lag handshake
// (StartADCLag / EndOfADCLag) for a single note-on per gate edge, and gates on
// edge transitions (read_gate vs gated), so a single bus gate edge yields one
// NoteOn, not ten. Tests clear the bus between steps so the rising edge is
// seen once (CLAUDE.md "Single-shot gate tests must clear the bus").
#include "../../shim/include/HemisphereApplet.h"
#include "../../shim/include/HemiPluginInterface.h"
#include "../../shim/include/applet_manifests/hMIDIOut.h"
#include "../../vendor/O_C-Phazerville/software/src/applets/hMIDIOut.h"
#include "_per_applet_runtime.h"

using Manifest = per_applet::hMIDIOut;
...
// ---------------------------------------------------------------------------
// Factory and plug-in entry point. hMIDIOut produces MIDI but consumes none,
// so midiMessage / midiRealtime are left null (default-initialised).
// ---------------------------------------------------------------------------
